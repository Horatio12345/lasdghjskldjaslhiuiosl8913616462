...
async def on_voice_state_update(member_before, member_after):
    ...
    if voice_channel_before == None:
        ...
        msg = "%s aka %s joined voice channel _%s_" % (member_after.display_name, member_after.name, voice_channel_after.name)
    else:
        ...
        if voice_channel_after == None:
            ...
            msg = "%s aka %s left voice channel _%s_" % (member_after.display_name, member_after.name, voice_channel_before.name)
        else:
            # The member is still on a voice channel after the change
            msg = "%s aka %s switched from voice channel _%s_ to _%s_" % (member_after.display_name, member_after.name, voice_channel_before.name, voice_channel_after.name)
